...
c     added by keke
     ...
     >                ,ju0,ju1,ju2,ju3,jf0,jar,jaa,jab,jac,jad,nar,jpid
      ...
      common /shareddata/ isGPU, num_sh, num_cores, shArray !used for the global to local transfer of element
c     end added by keke
...
c     added by keke to convert ipart(je0, i) to local element id
      if ( gfirst .eq. 0) then
         if(isGPU .and. (istep .ne. 0)) then
             call update_ipartje0_to_local_gpu  !in loadbal_cuda.cuf
         else
             call update_ipartje0_to_local_cpu  !in loadbal.f
         endif
      endif
